...
const InventoryEdit = (props) => {
    ...
    const handleSave = (e) => {
        e.preventDefault();
        props.parentCallback(formInput, props.data.id);
        setFormInput({
            item_name : '',
            item_qty : ''
        })
    ...
    return (
        <>
            <form className='flex flex-col sm:flex-row gap-2'>
                ...
                <ButtonAction color='blue' action={handleSave} disabled={!formValid}>Edit</ButtonAction>
                <ButtonAction color='red' action={handleCancelEdit}>Cancel</ButtonAction>
            </form>
        </>
    )
}
